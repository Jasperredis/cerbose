...
```
This means that `length`, `total`, and `fill` are required inputs and `percent` and `count` are optional keyword arguments.  
**Example**:  
```python
...
`percent` adds a percentage (e.g., `42/70` = `60.0%`) before or after the bar. It can be set to either `'l'` for the left of the bar, or `'r'` for the right of the bar.  
...
If both are present on the same side, percent is always on the far left (whether that be farther from the bar in left or closer in right), and obviously the opposite for count.
