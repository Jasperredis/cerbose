# Configuration
A lot of items in this documentation have referenced this very file for
configuration.  
Cerbose is configured using variables (and, optionally, wrapper functions) found
in `config.py`.
...
```
Use this structure for defining tags. Or, use the `config.add_tag` and/or
`config.remove_tag` functions:  
```python
...
tags = {
    ...
    "log":   {"text": "LOG",   "colour": "yellow"}
}
```

## Symbols
The symbols in Cerbose, such as the brackets on tags (**`[`**`OK`**`]`**) or the
space in `cerbar`, are customisable.  
...
```
> Note: As of Cerbose v2.0.0, `bracket_left` and `bracket_right` apply to both
the brackets on `cprint`/`cprint`-based functions *and* `cerbar`.

## Defaults
- `log_file`: Default file to log to. See `cprint.md`.
- `timestamp_default`: Whether to use a timestamp or not by default in cprint.
  See `cprint.md`.

## Other Settings
There are two other settings that you can configure in Cerbose:
- `config.space_repeat_tolerance`: See `cprint.md`.
- `config.time_format`: The `strftime` format for `cprint` timestamps.
  See https://docs.python.org/3/library/datetime.html#strftime-strptime-behavior
  for time formatting (it uses this function).
