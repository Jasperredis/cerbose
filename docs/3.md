...
```
and even adding `cerbar`, `cin`, `cyn` to it if you really need those functions a lot. Though, `cprint`/`mprint` are the functions you absolutely do not want to have to type `cerbose.` for. You may also want `config`.
